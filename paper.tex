...
\paragraph{The gain is stable across seeds.} We repeat the comparison over three training seeds with the dataset split held fixed (Table~\ref{tab:seeds}). Aligned pretraining averages $0.803 \pm 0.004$ accuracy and beats the no-pretraining baseline at \emph{every} seed ($+0.019$, $+0.054$, $+0.089$); the baseline is not only lower on average ($0.749 \pm 0.038$) but roughly an order of magnitude more variable. Predictive pretraining therefore both raises accuracy and stabilizes training. The released checkpoint (seed 42, $0.807$) lies at the top of the aligned range but within $0.004$ of the mean, so it is representative rather than cherry-picked.

\begin{table}[h]
  \caption{Accuracy across three training seeds (fixed data split). Aligned JEPA pretraining is both higher and far less variable than SupCon from a random initialization.}
  \label{tab:seeds}
  \centering
  \begin{tabular}{lccccc}
    \toprule
    Configuration & seed 42 & seed 1 & seed 2 & mean & std \\
    \midrule
    Aligned JEPA (ours)          & 0.807 & 0.799 & 0.802 & \textbf{0.803} & \textbf{0.004} \\
